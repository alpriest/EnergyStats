...
struct ResponseDebugView_Previews: PreviewProvider {
    ...
    static var previews: some View {
        ...
        Task {
            store.reportResponse = try NetworkOperation(description: "fetchReport", value: await network.fetchReport(deviceID: "123", variables: [.chargeEnergyToTal], queryDate: .now(), reportType: .day), raw: "Report is only fetched and cached on the graph view. Click that page to load report data\nClick that page to load report data".data(using: .utf8)!)
        }
